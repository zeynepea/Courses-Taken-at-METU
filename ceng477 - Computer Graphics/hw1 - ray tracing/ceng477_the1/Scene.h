...
private:
    Vector3f ambientShading(int mat);
    Vector3f diffuseShading(int hitObj, const Vector3f& w_i, const Vector3f& normal, const Vector3f& irr);
    Vector3f specularShading(int hitObj, const Vector3f& w_i, const Vector3f& normal,const Vector3f& w_o, const Vector3f& irr);
    Vector3f pixelColor(const Ray &ray,int maxDepth);
    bool check = false;
